...
def bjet_weight(df,ismc, is_inclusive, name,sample_weight, era, var_string='_jet_nom_muon_corrected_pt_ele_pt'):
    if int(ismc):
        ...
        #correlated
        df = df.Define("BTagWeightCorrUp", "map_zero_to_one(GetBTagWeight(GoodBJet{}, GoodJet{}, Jet_hadronFlavour, Jet_pt, Jet_btagSF_deepflavour_M_up_correlated))".format(var_string, var_string))
        df = df.Define("BTagWeightCorrDown", "map_zero_to_one(GetBTagWeight(GoodBJet{}, GoodJet{}, Jet_hadronFlavour, Jet_pt, Jet_btagSF_deepflavour_M_down_correlated))".format(var_string, var_string)) 
        #compute per jet sf
        df = df.Define("BTagWeightCorrUpPerEvent", "GetBTagWeightPerJet(GoodBJet{}, GoodJet{}, Jet_hadronFlavour, Jet_pt, Jet_btagSF_deepflavour_M_up_correlated)".format(var_string, var_string))
        df = df.Define("BTagWeightCorrPerEvent", "GetBTagWeightPerJet(GoodBJet{}, GoodJet{}, Jet_hadronFlavour, Jet_pt, Jet_btagSF_deepflavour_M)".format(var_string, var_string))
        df = df.Define('AvgBtagWeightCorr', 'CalcAverage(DivideVec(BTagWeightCorrUpPerEvent,BTagWeightCorrPerEvent))')
        
        #uncorrelated
        df = df.Define("BTagWeightUncorrUp", "map_zero_to_one(GetBTagWeight(GoodBJet{}, GoodJet{}, Jet_hadronFlavour, Jet_pt, Jet_btagSF_deepflavour_M_up_uncorrelated))".format(var_string, var_string))
        df = df.Define("BTagWeightUncorrDown", "map_zero_to_one(GetBTagWeight(GoodBJet{}, GoodJet{}, Jet_hadronFlavour, Jet_pt, Jet_btagSF_deepflavour_M_down_uncorrelated))".format(var_string, var_string)) 
        #compute per jet sf
        df = df.Define("BTagWeightUncorrUpPerEvent", "GetBTagWeightPerJet(GoodBJet{}, GoodJet{}, Jet_hadronFlavour, Jet_pt, Jet_btagSF_deepflavour_M_up_uncorrelated)".format(var_string, var_string))
        df = df.Define("BTagWeightUncorrPerEvent", "GetBTagWeightPerJet(GoodBJet{}, GoodJet{}, Jet_hadronFlavour, Jet_pt, Jet_btagSF_deepflavour_M)".format(var_string, var_string))
        df = df.Define('AvgBtagWeightUncorr', 'CalcAverage(DivideVec(BTagWeightUncorrUpPerEvent,BTagWeightUncorrPerEvent))')
    else:
        df = df.Define("BTagWeight", "1.")
        df = df.Define("BTagWeightCorrUp", "1.")
        df = df.Define("BTagWeightCorrDown", "1.")
        df = df.Define("BTagWeightUncorrUp", "1.")
        df = df.Define("BTagWeightUncorrDown", "1.")
        #compute per jet sf
        df = df.Define("BTagWeightCorrUpPerEvent", "1.")
        df = df.Define("BTagWeightCorrPerEvent", "1.")
        df = df.Define('AvgBtagWeightCorr', "1.")
        df = df.Define("BTagWeightUncorrUpPerEvent", "1.")
        df = df.Define("BTagWeightUncorrPerEvent", "1.")
        df = df.Define('AvgBtagWeightUncorr', "1.")
    return df
...
def finalize_weights(df,ismc, is_inclusive, name,sample_weight, era):
    if int(ismc):
        ...
        df = df.Define("Weight_BTagCorrUp", "Weight/BTagWeight*BTagWeightCorrUp")
        df = df.Define("Weight_BTagCorrDown", "Weight/BTagWeight*BTagWeightCorrDown")
        df = df.Define("Weight_BTagUncorrUp", "Weight/BTagWeight*BTagWeightUncorrUp")
        df = df.Define("Weight_BTagUncorrDown", "Weight/BTagWeight*BTagWeightUncorrDown")
